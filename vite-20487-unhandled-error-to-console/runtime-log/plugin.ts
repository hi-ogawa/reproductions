...
import { DevEnvironment, normalizePath, Plugin } from "vite";
import { parseErrorStacktrace } from "@vitest/utils/source-map";
import path from "node:path";
import fs from "node:fs";
import { generateCodeFrame } from "./vitest/printError";
  ...
  return {
    ...
    configureServer(server) {
      for (const name of environmentNames) {
        ...
        environment.hot.on("runtime-log", (payload) => {
    ...
    // TODO: how to inject for non client runtime?
    transformIndexHtml: {
      handler() {
        return [
          {
            tag: "script",
            attrs: { type: "module", src: "/@id/__x00__@vite/runtime-log" },
          },
        ];
      },
    },

    // virtual module to initialize runtime
    resolveId: {
      async handler(source) {
        if (source === "@vitejs/runtime-log") {
          return "\0" + source;
        }
      },
    },
    load: {
      handler(id) {
        if (id === "\0@vite/runtime-log") {
          if (!environmentNames.includes(this.environment.name)) {
            return "";
          }
          return `(${setupRuntime.toString()})()`;
        }
      },
    },
  };
...
function setupRuntime() {
  if (import.meta.hot) {
    function sendError(error: any) {
      // TODO: serialize better? (extra properties, recursively for cause)
      import.meta.hot!.send("runtime-log", {
        type: "error",
        error: {
          name: error.name,
