...
beforeAll(async () => {
  ...
  const flattedFile = process.env.FLATTED_FILE ?? resolve(cwd, "fixtures/blob-flatted.json");
  const devalueFile = process.env.DEVALUE_FILE ?? resolve(cwd, "fixtures/blob-devalue.json");
  const ungapFile = process.env.UNGAP_FILE ?? resolve(cwd, "fixtures/blob-ungap.json");
